...
\begin{document}
	...
	Intel® Fortran Composer XE 2011 Update 6\\ \href{http://imserv.eps.manchester.ac.uk/software/intel_fortran/win/compiler/12/w_fcompxe_2011.6.233.exe}{Download the installer}\\
	...
	\begin{enumerate}
		\item Documentation
			...
			\textbf{Note that installing the documentation is not optional - the solvers won't install unless the documentation has already been installed.}
	...
	Mount the documentation installer \path{Abaqus_fesafe_2016_documentation.iso}
	...
	\includegraphics[width=\textwidth]{"abaqus2016_documentation_complete_redbox".PNG}
	...
	\path{\3DEXPERIENCE_AbaqusSolver\Windows64\1\setup.exe}
	
	\subsection{CAA developer}
	Run the installer
	\path{\CAA_3DEXPERIENCE_AbaqusSolver\Windows64\1\setup.exe}
	
	\subsection{CAE}
	Run the installer
	\path{\SIMULIA_Abaqus_CAE\Windows64\1\setup.exe}
	...
	To get User Subroutines working with Intel Compiler XE 2011, copy \\ \path{abaqus_set_env.bat} to \path{C:\SIMULIA\} and then modify the Abaqus start menu shortcuts thus:
	\begin{enumerate}
